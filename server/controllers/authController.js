...
//         // Store the token in session and redirect to the protected resource
...
const handleGoogleCallback = async (req, res) => {
    console.log("Session oauthState:", req.session.oauthState); // Check if state exists in session

    // const { code, state } = req.query;
    const { code, state } = req.query;
    console.log("State received from Cognito:", state);
    console.log("State stored in session:", req.session.oauthState);


    // Check if the state matches what was stored in the session
    if (state !== req.session.oauthState) {
        console.error("State mismatch. Session state:", req.session.oauthState, "Returned state:", state);
