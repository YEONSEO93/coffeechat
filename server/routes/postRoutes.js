...
const { addPost, getPost } = require("../controllers/dynamoDBController"); // DynamoDB 관련 함수 추가
const { getPreSignedUrl, uploadFileToS3, deleteImageFromS3, getPreSignedReadUrl } = require('../controllers/s3Controller'); // S3 관련 함수들
const { getDB } = require('../config/db');

const router = express.Router();
const upload = multer({ storage: multer.memoryStorage() });  // 메모리 내에서 파일 저장

// 게시물 추가 라우트 (DynamoDB)
router.post("/addPost", ensureAuthenticated, upload.single('img1'), addPost); // img1로 파일 업로드

// 게시물 조회 라우트 (DynamoDB)
router.get("/getPost/:postId", getPost);

// 기존 라우트: Route to render the write post page
router.get('/write', ensureAuthenticated, (req, res) => {
    res.render('write', { user: req.user });
});

// 기존 라우트: Route to handle adding a new post
...
// 기존 라우트: Route to get details of a specific post by ID
...
// 기존 라우트: Route to render the edit page for a specific post by ID
router.get('/edit/:id', ensureAuthenticated, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to load the edit page' });
    }
});

// 기존 라우트: Route to handle updating a specific post by ID
...
// 기존 라우트: Route to handle deleting a specific post by ID
...
// 기존 라우트: Route to handle generating a pre-signed URL for an uploaded file
